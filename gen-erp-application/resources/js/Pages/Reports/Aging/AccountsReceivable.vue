      ...
      <AppLayout>
        <div class="py-6">
          <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-6">
              <h1 class="text-2xl font-semibold text-gray-900">Accounts Receivable Aging</h1>
              <p class="text-gray-600 mt-1">View your accounts receivable aging report</p>
            </div>
          ...
          <div class="p-6 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">AR Aging as of {{ formatDate(filters.as_of_date) }}</h2>
          </div>
          ...
          <div class="p-6">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tbody class="bg-white divide-y divide-gray-200">
                <tr v-for="customer in reportData.customers" :key="customer.customer_id">
                  <td class="px-4 py-3 text-sm text-gray-900">
                    <div class="font-medium">{{ customer.customer_name }}</div>
                    <div class="text-xs text-gray-500">{{ customer.customer_code }}</div>
                  </td>
                  <td class="px-4 py-3 text-right text-sm font-bold text-gray-900">{{ formatCurrency(customer.total_outstanding) }}</td>
                  <td class="px-4 py-3 text-right text-sm text-green-600">{{ formatCurrency(customer.current) }}</td>
                  <td class="px-4 py-3 text-right text-sm text-yellow-600">{{ formatCurrency(customer.days_1_30) }}</td>
                  <td class="px-4 py-3 text-right text-sm text-orange-600">{{ formatCurrency(customer.days_31_60) }}</td>
                  <td class="px-4 py-3 text-right text-sm text-red-600">{{ formatCurrency(customer.days_61_90) }}</td>
                  <td class="px-4 py-3 text-right text-sm text-red-800">{{ formatCurrency(customer.days_over_90) }}</td>
                  <td class="px-4 py-3 text-center text-sm text-gray-600">{{ customer.invoice_count }}</td>
                </tr>
              </tbody>
            </table>
          </div>
...
const generateReport = async () => {
  loading.value = true;
  try {
    const response = await fetch('/api/v1/reports/aging/ar', {
      method: 'POST',
...
const downloadPdf = () => {
  window.open(`/api/v1/reports/aging/ar/pdf?as_of_date=${filters.value.as_of_date}`, '_blank');
};

const downloadExcel = () => {
  window.open(`/api/v1/reports/aging/ar/excel?as_of_date=${filters.value.as_of_date}`, '_blank');
};
