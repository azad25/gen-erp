...
<div x-show="{{ $show }}"
     ...
     x-init="{{ $duration > 0 ? 'setTimeout(() => $wire.set(\'show\', false), ' . $duration . ')' : '' }}"
     style="
        position: fixed;
        bottom: 24px;
        right: 24px;
    ...
    <div style="flex: 1;">
        @if($title)
            <div style="font-weight: 600; color: {{ $config['text'] }}; margin-bottom: 2px; font-size: 14px;">
                {{ $title }}
            </div>
        @endif
        <div style="color: {{ $config['text'] }}; font-size: 13px; line-height: 1.4;">
            {{ $message ?? $slot }}
        </div>
    </div>
