...
class PayrollService
{
    ...
    private function getAttendanceCounts(Employee $employee, int $month, int $year): array
    {
        ...
        return [
            'present_days' => (float) $attendances->whereIn('status', [\App\Support\Enums\AttendanceStatus::PRESENT, \App\Support\Enums\AttendanceStatus::LATE])->count(),
            'absent_days' => (float) $attendances->where('status', \App\Support\Enums\AttendanceStatus::ABSENT)->count(),
            'leave_days' => (float) $attendances->where('status', \App\Support\Enums\AttendanceStatus::ON_LEAVE)->count(),
     ...
     */
    public function getPayslip(int $companyId, int $id): Payslip
    {
        return Payslip::where('company_id', $companyId)->findOrFail($id);
    }
     ...
     */
    public function getPayslipDownloadUrl(Payslip $payslip): string
    {
        // Generate a temporary signed URL for payslip download
        return route('api.v1.payslips.pdf', $payslip->id);
    }
}
