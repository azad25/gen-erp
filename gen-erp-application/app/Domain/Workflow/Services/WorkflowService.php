...
class WorkflowService
{
    ...
    private function handleNotifyRoles(array $action, WorkflowInstance $instance): void
    {
        ...
        $companyId = $instance->company_id;
        $users = \App\Domain\Auth\Models\CompanyUser::withoutGlobalScopes()
            ->where('company_id', $companyId)
            ->whereIn('role', $roles)
            ->where('is_active', true)
            ->get();
        ...
        foreach ($users as $companyUser) {
            $event = \App\Support\Enums\NotificationEvent::tryFrom('workflow_notification');
    ...
    {
        $modelMap = [
            'sales_order' => \App\Domain\SalesOrder\Models\SalesOrder::class,
            'purchase_order' => \App\Domain\Purchase\Models\PurchaseOrder::class,
            'invoice' => \App\Domain\Invoice\Models\Invoice::class,
