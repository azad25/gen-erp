...
class TwoFactorService
{
     ...
     */
    public function getQrCodeUrl(string $email, string $appName, string $secret): string
    {
     ...
     */
    public function generateCodeForTimestamp(string $secret, int $timestamp): string
    {
        $timeSlice = (int) floor($timestamp / 30);
        return $this->generateTotp($secret, $timeSlice);
    }

    /**
     * Validate TOTP code for a specific timestamp (for testing).
     */
    public function validateCode(string $secret, string $code, int $timestamp): bool
    {
        $timeSlice = (int) floor($timestamp / 30);
